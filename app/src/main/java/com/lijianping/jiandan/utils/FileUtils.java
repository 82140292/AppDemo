...
public class FileUtils {
    ...
    public static void savePicture(Activity activity, String picUrl, LoadFinishCallBack loadFinishCallBack) {
        ...
        File picDir = new File(CacheUtils.getSaveDirPath());

        if (!picDir.exists()) {
            picDir.mkdir();
        }

        final File newFile = new File(picDir, CacheUtils.getSavePicName(cacheFile, urls));
        ...
        if (FileUtils.copyTo(cacheFile, newFile)) {
            ...
            ToastUtils.Short(ConstantString.SAVE_FAILED);
        }
