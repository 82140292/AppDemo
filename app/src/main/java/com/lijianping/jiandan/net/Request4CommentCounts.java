...
import com.lijianping.jiandan.model.CommentNumber;
...
/**
 * @fileName: Request4CommentCounts
 * @Author: Li Jianping
...
public class Request4CommentCounts extends Request<ArrayList<CommentNumber>> {
    ...
    @Override
    protected Response<ArrayList<CommentNumber>> parseNetworkResponse(NetworkResponse networkResponse) {
        try {
            ...
            for (String comment_ID : comment_IDs){
                if (!jsonObject.isNull(comment_ID)){
                    ...
                    //可能会出现没有对应id的数据的情况，为了保证数据的一致，添加默认数据
                    commentNumbers.add(new CommentNumber("0", "0", 0));
                }

            }
            return Response.success(commentNumbers, HttpHeaderParser.parseCacheHeaders(networkResponse));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    protected void deliverResponse(ArrayList<CommentNumber> commentNumbers) {
        listener.onResponse(commentNumbers);
    }
}
